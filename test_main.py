...
from tool import gen_label, create_logits, get_acc, create_sim_matrix, gen_label_from_text_sim, get_m_theta, get_acc_v2, get_acc_v3
...
class Processor:
    ...
    @ex.capture
    def train_epoch(self, epoch, lr, loss_mode, step, loss_type, alpha, beta, m):
    ...
    def test_epoch(self, unseen_label, epoch, DA, support_factor):
        ...
        if acc > self.best_acc:
            ...
            # self.log.info('true_labels:{}'.format(y_true))
            # self.log.info('predicted_labels:{}'.format(y_pred))
            self.log.info('unseen_label:{}'.format(unseen_label))
            conf_matrix = confusion_matrix(y_true, y_pred, labels=unseen_label)
            self.log.info('Confusion Matrix:{}{}'.format('\n',conf_matrix))
        ...
        if DA:
            ...
            for data, label in tqdm(loader):
                ...
                # acc_batch, pred = get_acc(feature, unseen_language, unseen_label, label)
                # acc_batch, pred = get_acc(feature, unseen_language, unseen_label, label)
                acc_batch, pred, ent = get_acc_v3(feature, unseen_language, unseen_label, label)
                DA_ent_list.append(ent)
            
                y_p = pred.cpu().numpy().tolist()
                y_pred_DA += y_p

                aug_acc_list.append(acc_batch)
            mean_DA_ent = torch.mean(torch.cat(DA_ent_list))
            ...
            if aug_acc > self.best_aug_acc:
                ...
                # self.log.info('true_labels:{}'.format(y_true))
                # self.log.info('DA_predicted_labels:{}'.format(y_pred_DA))
                self.log.info('Unseen label:{}'.format(unseen_label))
                conf_matrix = confusion_matrix(y_true, y_pred_DA, labels=unseen_label)
                self.log.info('DA Confusion Matrix:{}{}'.format('\n',conf_matrix))
                self.log.info('Mean old entropy:{}'.format(mean_old_ent))
                self.log.info('Mean DA entropy:{}'.format(mean_DA_ent))
